...
export default function AdminDashboardPage() {
  ...
  const [material, setMaterial] = useState('100% Premium Dacron Silikon & Kain Rasfur Halus');
  const [sizeSpec, setSizeSpec] = useState('Ukuran Standar');
      ...
      if (data && data.length > 0) {
        ...
        const mappedData = data.map((item: any) => {
          ...
          return {
            ...
            specifications: {
              material: specs.material || '100% Premium Dacron Silikon & Kain Rasfur Halus',
              size: specs.size || 'Ukuran Standar',
              washing: specs.washing || 'Bisa dicuci dengan mesin (putaran lembut)',
              ...
              types: specs.types || undefined,
              sizes: specs.sizes || undefined,
            },
        ...
        localStorage.setItem('simoengil_mock_products', JSON.stringify(mappedData));
        setProducts(mappedData);
      } else {
        const localMock = localStorage.getItem('simoengil_mock_products');
        if (localMock) {
          setProducts(JSON.parse(localMock));
        } else {
  ...
  const resetForm = () => {
    ...
    setRating('5.0');
    setReviewsCount('0');
    setMaterial('100% Premium Dacron Silikon & Kain Rasfur Halus');
    setSizeSpec('Ukuran Standar');
    setShopeeLink('');
    setShopeePrice('');
  ...
  const handleTypeChange = (index: number, key: keyof ProductType, value: any) => {
    ...
    const isNumeric = ['extraPrice'].includes(key);
    updated[index] = {
      ...updated[index],
  ...
  const removeTypeRow = (index: number) => {
    setFormTypes(formTypes.filter((_, i) => i !== index));
  };

  // Add a new size row
  const addSizeRow = () => {
    setFormSizes([...formSizes, { name: '', extraPrice: 0 }]);
  };

  const handleSizeChange = (index: number, key: keyof ProductSize, value: any) => {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      shopee_link: shopeeLink || 'https://shopee.co.id',
      shopeeAvailable,
      ...
      variants: [] // Kept empty for backwards compatibility with DB schema
    };
    ...
    try {
      ...
      if (editingId) {
        const updated = products.map(p => p.id === editingId ? fallbackProduct as any : p);
        setProducts(updated);
        localStorage.setItem('simoengil_mock_products', JSON.stringify(updated));
        alert(`💾 Disimpan di Memory Lokal (Supabase offline/belum setup): ${err.message}`);
      } else {
        const updated = [...products, fallbackProduct as any];
        setProducts(updated);
        localStorage.setItem('simoengil_mock_products', JSON.stringify(updated));
  ...
  const handleEditProduct = (p: Product) => {
    ...
    const derivedTypes = p.specifications?.types || (p.variants ? Array.from(new Set(p.variants.map((v: any) => v.type).filter(Boolean)))
      ...
      : []);
    const derivedSizes = p.specifications?.sizes || (p.variants ? Array.from(new Set(p.variants.map((v: any) => v.size).filter(Boolean)))
      .map((s) => ({ name: s as string, extraPrice: 0 }))
      : []);
    setFormTypes(derivedTypes);
    setFormSizes(derivedSizes);
    ...
    setSoldCount(String(p.specifications?.soldCount || 0));
    setTestimonials(p.specifications?.testimonials || []);
  };
            ...
            <div className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm p-6 space-y-6">
              ...
              <form onSubmit={handleSaveProduct} className="space-y-4">
                ...
                <div className="bg-slate-50/60 rounded-2xl border border-slate-200 p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 block">Bahan Utama</label>
                      <input
                        type="text"
                        placeholder="100% Premium Dacron..."
                        value={material}
                        onChange={(e) => setMaterial(e.target.value)}
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:border-pink-400 focus:ring-1 focus:ring-pink-100 transition-all"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-400 block">Dimensi / Ukuran</label>
                      <input
                        type="text"
                        placeholder="Tinggi 40cm, Lebar 20cm"
                        value={sizeSpec}
                ...
                <div className="bg-slate-50/60 rounded-2xl border border-slate-200 p-4 space-y-3">
                  ...
                  <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                    {features.length === 0 ? (
                      <p className="text-[10px] text-slate-400 italic text-center py-3 bg-white/50 border border-dashed border-slate-200 rounded-xl">
                        Belum ada kelebihan. Klik &quot;Tambah Kelebihan&quot; di atas.
                      </p>
                    ) : (
                      features.map((feat, idx) => (
                        <div key={idx} className="bg-white border border-slate-200 rounded-xl p-3 flex items-start gap-3 relative">
                          <input
                            type="text"
                            placeholder="Emoji 🪡"
                            value={feat.icon}
                            ...
                            className="w-12 text-center px-1 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-lg focus:outline-none focus:border-pink-400 transition-all"
                          />
                          <div className="flex-1 space-y-2">
                            <input
                              type="text"
                              placeholder="Judul Kelebihan (Misal: Jahitan Super Kuat)"
                              value={feat.title}
                              onChange={(e) => {
                                const updated = [...features];
                              ...
                              className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-800 focus:outline-none focus:border-pink-400 transition-all"
                            />
                            <textarea
                              rows={2}
                              ...
                              className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-medium text-slate-600 focus:outline-none focus:border-pink-400 transition-all resize-none"
                            />
                          </div>
                          <button
                            type="button"
                            onClick={() => setFeatures(features.filter((_, i) => i !== idx))}
                            className="p-1.5 bg-rose-50 text-rose-400 hover:text-rose-600 hover:bg-rose-100 rounded-lg transition-colors absolute top-3 right-3"
                ...
                <div className="bg-slate-50/60 rounded-2xl border border-slate-200 p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setTestimonials([...testimonials, { name: '', rating: 5, message: '', date: '', avatar: '' }])}
                      className="flex items-center gap-1 text-[10px] font-extrabold text-blue-500 hover:text-blue-600 bg-blue-50 border border-blue-100 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer"
                    >
                      <Plus className="w-3 h-3" />
                  ...
                  <div className="space-y-3 max-h-72 overflow-y-auto pr-1">
                    {testimonials.length === 0 ? (
                      <p className="text-[10px] text-slate-400 italic text-center py-3 bg-white/50 border border-dashed border-slate-200 rounded-xl">
                        Belum ada testimoni. Klik &quot;Tambah Testimoni&quot; di atas.
                      </p>
                    ) : (
                      testimonials.map((testi, idx) => (
                        <div key={idx} className="bg-white border border-slate-200 rounded-xl p-3 pr-10 flex flex-col gap-2 relative">
                          <div className="flex flex-wrap sm:flex-nowrap gap-2">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                updated[idx].avatar = e.target.value.substring(0, 2).toUpperCase();
                                setTestimonials(updated);
                              }}
                              className="flex-1 min-w-[120px] px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-800 focus:outline-none focus:border-pink-400 transition-all"
                            />
                            <div className="w-24 shrink-0">
                              <input
                                ...
                                className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:outline-none focus:border-pink-400 transition-all"
                              />
                            </div>
                            <input
                              type="text"
                              ...
                              className="w-24 shrink-0 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-800 focus:outline-none focus:border-pink-400 transition-all"
                            />
                          </div>
                          <textarea
                            rows={2}
                          ...
                          <button
                            ...
                            className="p-1.5 bg-rose-50 text-rose-400 hover:text-rose-600 hover:bg-rose-100 rounded-lg transition-colors absolute top-2 right-2"
                      ...
                      formTypes.map((t, i) => (
                        <div key={i} className="bg-slate-50/60 rounded-2xl border border-slate-200/60 p-3 space-y-2 relative">
                          ...
                          <div className="space-y-1 mt-2">
                            <span className="text-[9px] font-bold text-slate-400">Foto Variasi (Opsional)</span>
                            <div className="flex items-center gap-2 w-full">
                              <input
                                type="text"
                                placeholder="URL Foto atau Upload"
                                value={t.image || ''}
                                onChange={(e) => handleTypeChange(i, 'image', e.target.value)}
                                className="flex-1 px-2 py-1 bg-white border border-slate-200 focus:border-pink-300 rounded-lg text-[10px] font-medium text-slate-800"
                              />
                              {t.image && (
                                <div className="relative w-6 h-6 rounded border border-slate-200 bg-white shrink-0 overflow-hidden">
                                  <img src={t.image} alt={`Preview Var ${i + 1}`} className="w-full h-full object-cover" />
                                </div>
                              )}
                              <label title="Upload Foto Variasi" className="shrink-0 p-1.5 bg-slate-100 hover:bg-pink-100 text-slate-400 hover:text-pink-600 rounded-lg transition-colors cursor-pointer">
                ...
                <div className="border-t border-slate-100 pt-4 space-y-3">
                  ...
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                    {formSizes.length === 0 ? (
                      <p className="text-[10px] text-slate-400 text-center italic bg-slate-50/50 py-3 border border-dashed border-slate-200 rounded-xl">
                        Belum ada ukuran. Tambahkan agar pembeli dapat memilih varian ukuran.
                      </p>
                    ) : (
                      formSizes.map((s, i) => (
                        <div key={i} className="bg-slate-50/60 rounded-2xl border border-slate-200/60 p-3 space-y-2 relative">
                          ...
                          <span className="text-[10px] font-black text-slate-400">Ukuran #{i + 1}</span>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                              <span className="text-[9px] font-bold text-slate-400">Nama Ukuran</span>
                              <input
                                type="text"
                                placeholder="11cm, Besar"
                                value={s.name}
                            ...
                            <div className="space-y-1">
                              ...
                              <input
                                ...
                                placeholder="0"
                                value={s.extraPrice || ''}
                                onChange={(e) => handleSizeChange(i, 'extraPrice', e.target.value)}
                                className="w-full px-2 py-1 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-800"
                              />
                            </div>
                          </div>
